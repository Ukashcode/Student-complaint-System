...
export default function StudentRegister() {
  ...
  const { login } = useAuth();
  const [form, setForm] = useState({ name: '', registrationNumber: '', email: '', phoneNumber: '', password: '' });
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f7f5', fontFamily: '"DM Sans", sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px' }}>
        <div style={{ backgroundColor: 'white', borderRadius: '24px', border: '1px solid #e5e7eb', padding: '36px', width: '100%', maxWidth: '420px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <h2 style={{ fontFamily: '"Playfair Display", serif', fontSize: '28px', fontWeight: 'bold', color: '#0f1f17', marginBottom: '4px' }}>Create Account</h2>
          <p style={{ color: '#9ca3af', fontSize: '13px', marginBottom: '24px' }}>Join FUD-RESOLVE as a student</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {[
              ...
              { label: 'Reg. Number', name: 'registrationNumber', type: 'text', placeholder: 'e.g. FUD/2023/001' },
              { label: 'Email Address', name: 'email', type: 'email', placeholder: 'your@email.com' },
              { label: 'Phone Number', name: 'phoneNumber', type: 'tel', placeholder: '08012345678' },
          ...
          <p style={{ textAlign: 'center', color: '#9ca3af', fontSize: '13px', marginTop: '20px' }}>
            ...
            <Link to="/login" style={{ color: '#1a6b3c', fontWeight: '600', textDecoration: 'none' }}>Login</Link>
          </p>
        </div>
      </div>
    </div>
